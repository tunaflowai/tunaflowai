...
<aside class="sidebar">
  <div class="brand">
    ...
    <div><h1>TunaFlowAI</h1><small>Pusat kontrol agen lokal</small></div>
  </div>
  <div class="nav" id="nav"></div>
  <div class="sidebar-card"><b>Sesi</b><span class="muted" id="authBadge">${authBadge}</span><div class="field" style="margin-top:10px"><label>Token Bearer</label><input class="input" id="tokenInput" type="password" placeholder="Tempel token lokal" /></div><button class="btn secondary" id="saveToken">Simpan token</button></div>
  <div class="sidebar-card"><b>Runtime</b><div class="muted" id="runtimeSummary">Memuat...</div><div class="wave"></div></div>
</aside>
<main class="main">
  <div class="topbar">
    <div><div class="kicker">Bertema Tuna, mirip Bootstrap, tanpa dependency</div><div class="title">Kendalikan semua channel dari satu GUI ringan.</div><p class="subtitle">Pilih model, kirim event channel, cek approval, kelola skill, tinjau audit log, dan tetap punya jalur CLI untuk tugas lokal berat.</p></div>
    <div class="status-pill"><span class="dot"></span><span id="statusText">Menghubungkan</span></div>
  </div>
  <section class="panel active" data-panel="overview"><div class="grid" id="overviewGrid"></div></section>
  <section class="panel" data-panel="chat"><div class="grid"><div class="card large"><h2>Chat dan perintah channel</h2><div class="split"><div class="field"><label>Model</label><select id="modelSelect"></select></div><div class="field"><label>Rantai</label><select id="chainSelect"></select></div></div><div class="field"><label>Pesan</label><textarea id="chatText" placeholder="Minta TunaFlowAI melakukan sesuatu..."></textarea></div><div class="row"><button class="btn" id="sendChat">Kirim ke TunaFlowAI</button><button class="btn secondary" id="refreshChatData">Muat ulang model</button></div></div><div class="card"><h2>Respons</h2><div class="code" id="chatResult">Belum ada respons.</div></div><div class="card full"><h2>Sisipkan event</h2><div class="split"><div class="field"><label>Tipe event</label><input class="input" id="eventType" value="user.message" /></div><div class="field"><label>Prioritas</label><select id="eventPrioritas"><option>normal</option><option>high</option><option>low</option></select></div></div><div class="field"><label>Teks event</label><textarea id="eventText" placeholder="Teks event"></textarea></div><button class="btn secondary" id="postEvent">Kirim event</button></div></div></section>
  <section class="panel" data-panel="models"><div class="grid"><div class="card full"><div class="toolbar"><h2>Katalog dan kesehatan model</h2><button class="btn secondary" id="refreshModel">Muat ulang</button></div><div id="modelsView"></div></div></div></section>
  <section class="panel" data-panel="skills"><div class="grid"><div class="card full"><div class="toolbar"><h2>Skill dan tool</h2><button class="btn secondary" id="refreshSkills">Muat ulang</button></div><div class="split"><div><h3>Skill bawaan</h3><div class="list" id="skillsList"></div></div><div><h3>Tool terdaftar</h3><div class="list" id="toolsList"></div></div></div></div></div></section>
  <section class="panel" data-panel="approvals"><div class="grid"><div class="card full"><div class="toolbar"><h2>Approval</h2><button class="btn secondary" id="refreshApproval">Muat ulang</button></div><div class="list" id="approvalsList"></div></div></div></section>
  <section class="panel" data-panel="operations"><div class="grid"><div class="card"><h2>Task</h2><div class="list" id="tasksList"></div></div><div class="card"><h2>Channel</h2><div class="list" id="channelsList"></div></div><div class="card"><h2>Secret</h2><div class="list" id="secretsList"></div></div><div class="card full"><h2>Run</h2><div class="list" id="runsList"></div></div></div></section>
  <section class="panel" data-panel="logs"><div class="grid"><div class="card large"><h2>Event</h2><div class="list" id="eventsList"></div></div><div class="card"><h2>Audit</h2><div class="list" id="auditList"></div></div></div></section>
</main>
</div>
...
(function(){
  ...
  var labels = {overview:'Ringkasan',chat:'Chat dan event',models:'Model',skills:'Skill dan tool',approvals:'Approval',operations:'Operasi',logs:'Event and audit'};
  ...
  function toast(message){ var el=$('toast'); el.textContent=message; el.classNama='toast show'; setTimeout(function(){ el.classNama='toast'; }, 3600); }
  ...
  function renderList(id, rows, mapper){ var el=$(id); if(!el) return; var list=Array.isArray(rows)?rows:[]; if(!list.length){ el.innerHTML='<div class="item muted">Belum ada data.</div>'; return; } el.innerHTML=list.map(mapper).join(''); }
  function renderRingkasan(data){ var overview=data.overview||{}; var approvals=data.approvals||[]; var skills=data.skills||[]; var tools=data.tools||[]; var models=(data.catalog&&data.catalog.configuredModel)||[]; $('runtimeSummary').textContent = 'Skills '+skills.length+' / Tools '+tools.length+' / Model '+models.length; $('overviewGrid').innerHTML = [
    ['Approval tertunda', approvals.length, 'Aksi berisiko yang menunggu persetujuan.'],
    ['Model terkonfigurasi', models.length, 'OpenAI, Codex OAuth, lokal, dan provider kompatibel.'],
    ['Skill bawaan', skills.length, 'Pekerjaan berbasis peran dan workflow otomasi.'],
    ['Tool terdaftar', tools.length, 'Aksi lokal aman, dry-run, dan petunjuk command.'],
    ['Persona aktif', overview.activePersona || 'default', 'Pemilihan rantai sadar persona.'],
    ['Status agen', overview.status || 'ready', 'Gateway lokal dapat diakses.']
  ...
  function renderModel(catalog, health){ state.catalog=catalog; state.models=health; var configured=catalog.configuredModel || catalog.models || []; var chains=catalog.chains || {}; var healthMap={}; ((health&&health.models)||[]).forEach(function(m){ healthMap[m.name]=m; }); var rows = configured.map(function(m){ var h=healthMap[m.name]||{}; return '<tr><td><strong>'+esc(m.name)+'</strong><br><span class="muted">'+esc(m.provider)+'</span></td><td>'+esc(m.model||m.codexModel||'default')+'</td><td><span class="badge '+esc(h.status||'')+'">'+esc(h.status|| (m.enabled===false?'disabled':'ready'))+'</span></td><td>'+esc((m.capabilities||[]).join(', '))+'</td></tr>'; }).join(''); $('modelsView').innerHTML = '<h3>Model terkonfigurasi</h3><table class="table"><thead><tr><th>Nama</th><th>Model</th><th>Status</th><th>Kapabilitas</th></tr></thead><tbody>'+rows+'</tbody></table><h3>Rantais</h3><div class="list">'+Object.keys(chains).map(function(k){return '<div class="item"><strong>'+esc(k)+'</strong><span class="muted">'+esc(chains[k].join(' -> '))+'</span></div>';}).join('')+'</div>';
    var modelOptions = ['<option value="">Gunakan default rantai</option>'].concat(configured.map(function(m){ return '<option value="'+esc(m.name)+'">'+esc(m.name+' ('+(m.provider||'provider')+')')+'</option>'; })); $('modelSelect').innerHTML = modelOptions.join('');
  ...
  async function postEvent(){ var model=$('modelSelect').value; var body={type:$('eventType').value||'user.message', priority:$('eventPrioritas').value||'normal', text:$('eventText').value||'', payload:{source:'dashboard'}}; if(model){ body.payload.model=model; body.payload.chain=model; } try{ var result=await api('/events',{method:'POST',body:JSON.stringify(body)}); toast('Event posted: '+(result.id||'ok')); await refreshCore(); }catch(err){ toast(err.message); } }
  function init(){ initNav(); $('tokenInput').value=state.token; $('saveToken').onclick=function(){ state.token=$('tokenInput').value.trim(); localStorage.setItem('tunaflow.token',state.token); toast('Token saved locally.'); refreshCore(); }; $('sendChat').onclick=sendChat; $('postEvent').onclick=postEvent; ['refreshModel','refreshSkills','refreshApproval','refreshChatData'].forEach(function(id){ var el=$(id); if(el) el.onclick=refreshCore; }); refreshCore(); setInterval(refreshCore, 30000); }
  init();
})();
</script>
</body>
