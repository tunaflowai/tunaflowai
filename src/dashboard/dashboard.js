...
(function(){
  ...
  function initNav(){ $('nav').innerHTML = panels.map(function(name){ return '<button data-panel="'+name+'">'+labels[name]+'</button>'; }).join(''); document.querySelectorAll('#nav button').forEach(function(b){ b.addEventsListener('click', function(){ setPanel(b.dataset.panel); }); }); setPanel(localStorage.getItem('tunaflow.panel') || 'overview'); }
  ...
  function renderOverview(data){ var overview=data.overview||{}; var approvals=data.approvals||[]; var skills=data.skills||[]; var tools=data.tools||[]; var models=(data.catalog&&data.catalog.configuredModel)||[]; $('runtimeSummary').textContent = 'Skills '+skills.length+' / Tools '+tools.length+' / Model '+models.length; $('overviewGrid').innerHTML = [
