...
export class SecretsVault {
  ...
  decrypt(record) { if (!record.encrypted) return record.value || ''; const key = this.key(); if (!key) throw new Error(`Secret is encrypted but ${this.keyEnv} is not set`); const decipher = crypto.createDecipheriv('aes-256-gcm', key, Buffer.from(record.iv, 'base64')); decipher.setAuthTag(Buffer.from(record.tag, 'base64')); return Buffer.concat([decipher.update(Buffer.from(record.ciphertext, 'base64')), decipher.final()]).toString('utf8'); }
}
