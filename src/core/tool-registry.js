...
export class ToolRegistry {
  ...
  registerFinanceTools() {
    this.register({ name: 'calculate_tax_ppn', description: 'Calculate VAT/PPN or any percentage tax from transaction amounts.', risk: 'low', schema: { required: ['amounts'], properties: { amounts: 'number[]', rate: 'number?' } }, execute: async ({ amounts, rate = 0.11 }) => { const values = asNumbers(amounts); const subtotal = values.reduce((a, b) => a + b, 0); const tax = subtotal * Number(rate); return { ok: true, subtotal, rate: Number(rate), tax, total: subtotal + tax }; } });
